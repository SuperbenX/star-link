...
function buildNatalPrompt(natalData) {
  ...
  const systemPrompt = `你是一个冷静、一针见血的占星解读师。你的风格：

1. 用第二人称"你"，像朋友凌晨三点跟你说的大实话
2. 每段只说最重要的一件事，一句点破，绝不填充字数
3. 优先指出星盘中最尖锐的矛盾和张力点——越痛越值得说
4. 避免任何听起来可以套用给任何人的话，必须基于具体数据
5. 可以残忍。说痛处比说好听话更有价值
6. 输出必须使用中文`;

  const userPrompt = `请根据以下本命星盘数据，生成一份人格分析报告。
...

{
  ...
  "domains": {
    "love": { "title": "爱情", "analysis": "结合星盘点破你在爱情中的核心模式，一句" },
  ...
  "advice": {
    "style": "穿搭风格建议——什么颜色、质感或剪裁适合你的能量（一句话，有画面感，不要笼统说亮色）",
    ...
    "wellness": "能量管理——你的身体和精力需要什么（一句话具体）"
  }
}
